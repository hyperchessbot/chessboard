var Chessground = (function () {
  ...
  var util = createCommonjsModule(function (module, exports) {
    ...
    exports.allKeys = Array.prototype.concat(
      ...types.files.map((c) => types.ranks.map((r) => c + r))
    );
  ...
  var board = createCommonjsModule(function (module, exports) {
    ...
    function tryAutoCastle(state, orig, dest) {
      ...
      if (origPos[0] === 4 && !state.pieces.has(dest)) {
        if (destPos[0] === 6) dest = util.pos2key([7, destPos[1]]);
        else if (destPos[0] === 2) dest = util.pos2key([0, destPos[1]]);
      }
    ...
    function setSelected(state, key) {
      ...
      if (isPremovable(state, key)) {
        state.premovable.dests = premove_1.premove(
          state.pieces,
          key,
    ...
    function canPremove(state, orig, dest) {
      return (
        ...
        premove_1
          .premove(state.pieces, orig, state.premovable.castle)
          .includes(dest)
      );
    }
  ...
  var draw = createCommonjsModule(function (module, exports) {
    ...
    function processDraw(state) {
      requestAnimationFrame(() => {
        ...
        if (cur) {
          const keyAtDomPos = board.getKeyAtDomPos(
            cur.pos,
            board.whitePov(state),
            state.dom.bounds()
          );
  ...
  var api = createCommonjsModule(function (module, exports) {
    ...
    function start(state, redrawAll) {
      ...
      return {
        set(config$1) {
          if (
            config$1.orientation &&
            config$1.orientation !== state.orientation
          )
            toggleOrientation();
          (config$1.fen ? anim_1.anim : anim_1.render)(
            (state) => config.configure(state, config$1),
            state
          );
        },
        state,
  ...
  var svg = createCommonjsModule(function (module, exports) {
    ...
    function shapeHash(
      { orig, dest, brush, piece, modifiers },
      arrowDests,
      current,
      bounds
    ) {
    ...
    function renderShape(
      ...
      if (shape.piece)
        el = renderPiece(
          ...
          orient(util.key2pos(shape.orig), state.orientation),
          shape.piece,
          bounds
        );
      else {
        ...
        if (shape.dest) {
          ...
          el = renderArrow(
            brush,
            orig,
            orient(util.key2pos(shape.dest), state.orientation),
            current,
  ...
  var render_1 = createCommonjsModule(function (module, exports) {
    ...
    function render(s) {
      const asWhite = board.whitePov(s),
        posToTranslate = s.dom.relative
          ...
          : util$1.posToTranslateAbs(s.dom.bounds()),
    ...
    function updateBounds(s) {
      ...
      while (el) {
        if ((isPieceNode(el) && !el.cgAnimating) || isSquareNode(el)) {
          util$1.translateAbs(
            el,
            posToTranslate(util.key2pos(el.cgKey), asWhite)
          );
        }
        el = el.nextSibling;
      }
    ...
    function computeSquareClasses(s) {
      ...
      const o = s.exploding;
      if (o)
        for (const k of o.keys) addSquare(squares, k, "exploding" + o.stage);
      return squares;
    }
